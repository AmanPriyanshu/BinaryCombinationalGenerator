...
class GeneticFormulaGenerator:
	def __init__(self, operators, operands, target, max_genome_sequence=10, min_genome_sequence=None, max_iters_sequence_resolver=100, lamda=0.1):
		self.operators = operators
		self.lamda = lamda
		self.target = target
		self.popularity_sequence = np.argsort(np.sum(self.target, 0))[::-1]
		self.target = np.argmax(self.target, 1)
	...
	def compute_loss(self, sequence):
		...
		for row in out:
			flag = False
			for popularity in self.popularity_sequence:
				if row[popularity] == 1:
					pred.append(popularity)
					flag = True
					break
			if not flag:
				pred.append(self.popularity_sequence[0])
		pred = np.array(pred)
		accuracy = np.where(pred==self.target, 1, 0)
		return 1-np.mean(accuracy)+(len(self.get_all_operands(sequence))-1)*self.lamda
	...
	def search(self, pop_size=1000, gen_iters=10, use_base=False):
		if use_base:
			pop = [i for i in range(len(self.operands))]+[self.generate_single_random_sequence() for _ in range(pop_size-len(self.operands))]
		else:
